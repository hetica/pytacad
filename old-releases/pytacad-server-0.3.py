...
def getInfosClasse(br):
	...
	for link in br.links():
		...
		# if "courseId=139700" in link.url:
	...
	for a in urls:
		...
		stag = {}
		nb = 0
		for prenom in soup.findAll('td', attrs={"headers" : "_omni_WAR_omniportlet_students_col-firstname" }):
			nb += 1
			stag[nb] = [ prenom.text.strip().encode('utf-8') ]
		nb = 0
		for nom in soup.findAll('td', attrs={"headers" : "_omni_WAR_omniportlet_students_col-lastname" }):
			nb += 1
			stag[nb].append(nom.text.strip().encode('utf-8'))
		nb = 0
		for nom_login in soup.findAll('td', attrs={"headers" : "_omni_WAR_omniportlet_students_col-screen-name" }):
			nb += 1
			stag[nb].append(nom_login.text.strip().encode('utf-8'))
		nb = 0
		for email in soup.findAll('td', attrs={"headers" : "_omni_WAR_omniportlet_students_col-email" }):
			nb += 1
			stag[nb].append(email.text.strip().encode('utf-8'))
		nb = 0
		for lastlogin in soup.findAll('td', attrs={"headers" : "_omni_WAR_omniportlet_students_col-last-login" }):
			nb += 1
			stag[nb].append(lastlogin.text.strip().encode('utf-8'))
