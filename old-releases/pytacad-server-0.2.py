		...
		for cadre in soup.findAll('div', attrs={ 'class': "portlet-borderless-container" }):
			...
			for table in cadre.findAll('table'):
				...
				if nb == 3:
					...
					id = table.find('th', {'class': "col-1 first"})
					...
					for a in table.findAll('tr'):
						for b in a.findAll('td' , {'headers' : _prenom }):
							prenom = b.text.strip().encode('utf-8')
						for b in a.findAll('td' , {'headers' : _nom }):
							nom =  b.text.strip().encode('utf-8')
						for b in a.findAll('td' , {'headers' : _login }):
							login = b.text.strip().encode('utf-8')
						for b in a.findAll('td' , {'headers' : _mail }):
							mail =  b.text.strip().encode('utf-8')
							nb_stag +=1
						for b in a.findAll('td' , {'headers' : _last_login }):
